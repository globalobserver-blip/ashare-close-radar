fetch('data/market.json?v=6')
  ...
  .then(data => {
    ...
    const direction = value => number(value) > 0 ? 'rise' : number(value) < 0 ? 'fall' : 'flat';
    ...
    fillMetric('shanghai', '上证指数', indexValue(shanghai.close), pct(shanghai.pct_chg), direction(shanghai.pct_chg));
    fillMetric('shenzhen', '深证成指', indexValue(shenzhen.close), pct(shenzhen.pct_chg), direction(shenzhen.pct_chg));
    ...
    const render = mode => {
      ...
      const card = item => { const metric = value(item), metricDirection = direction(metric), primary = flow ? `${metric > 0 ? '+' : ''}${number(metric).toFixed(1)}<small class="flow-unit">亿</small>` : pct(item.pct_chg); const detail = flow ? `涨跌 ${pct(item.pct_chg)} · ${item.count}股` : `${item.amount_billion}亿 · ${item.count}股`; const intensity = Math.max(.12, Math.abs(metric) / largest); const tile = metricDirection === 'rise' ? 'rise-tile' : metricDirection === 'fall' ? 'fall-tile' : 'flat-tile'; return `<a class="sector-card ${tile}" style="--shade:${Math.round(intensity * 15)}%;--bar:${Math.round(22 + intensity * 74)}%" href="#industry" data-industry="${item.name}"><b>${item.name}</b><strong>${primary}</strong><small>${detail}</small></a>`; };
